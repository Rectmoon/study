...
2. 子查询一般放在条件的右侧
3. 标量子查询一般搭配单行操作符使用(>、=、<、>=、<=、<>)
4. 列子查询一般搭配多行操作符使用(in/not in、 any/some、all)
5. 子查询的执行顺序优先于主查询，主查询的条件用到了子查询的结果
...
SELECT MIN(salary) ms, department_id
FROM employees
GROUP BY department_id
HAVING ms > (
    SELECT MIN(salary) 
...
ON av_dep.ag BETWEEN g.lowest_sal AND g.highest_sal;
```
...
SELECT * 
FROM employees
WHERE commision_pac IS NOT NULL
...
- UNION 关键字默认去重，使用UNION ALL 可以包含重复项目
...
#6. 查询公司所有manager的员工编号
SELECT * 
FROM employees 
WHERE manager_id IN (
	SELECT DISTINCT manger_id
	FROM employees
);

#7. 查询各个部门最高工资中最低的那个部门的最低工资
